...
import java.net.URI
import geekbang.spark.homework.objects.{Accumulators, ConfigSerDeser, CopyDefinitionWithDependencies, CopyPartitioner, DistCPResult, FileSystemObjectCacher, Logging}
import geekbang.spark.homework.utils.{CopyUtils, FileListUtils, PathUtils}
...
object SparkDistCP extends Logging {
  ...
  def run(
    ...
    //set max concurrence task
    sparkSession.sparkContext.hadoopConfiguration.set("spark.default.parallelism", String.valueOf(options.maxTask))
  ...
  /** DistCP helper implicits on iterators
    */
  private[homework] implicit class DistCPIteratorImplicit[B](
    iterator: Iterator[B]
  ) {
  ...
  /** Batch the given RDD into groups of files depending on
    ...
    */
  private[homework] def batchAndPartitionFiles(
  ...
  /** Key the RDD within partitions based on batches of files based on
    ...
    */
  private[homework] def generateBatchedFileKeys(
    maxFilesPerTask: Int,
    maxBytesPerTask: Long
