package geekbang.spark.homework.utils

import java.io.FileNotFoundException
import java.net.URI
import geekbang.spark.homework.SparkDistCPOptions
import geekbang.spark.homework.objects.CopyActionResult.SkippedAlreadyExists
import geekbang.spark.homework.objects.DeleteActionResult.{Deleted, SkippedDoesNotExists, SkippedDryRun}
import geekbang.spark.homework.objects.{CopyActionResult, DeleteActionResult, DeleteResult, DirectoryCopyResult, DistCPResult, FileCopyResult, Logging, SerializableFileStatus, SingleCopyDefinition}
  ...
  private[utils] def deleteFile(
    ...
    if (!fs.exists(path)) {
      ...
      Try(fs.delete(path, true)) match {
        case Success(true) =>
          DeleteResult(path.toUri, Deleted)
